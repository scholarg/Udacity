...
#         self.activation_function = lambda x : 0  # Replace 0 with your sigmoid calculation.
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            error = y - final_inputs # Output layer error is the difference between desired target and actual output.
...
# 
# #### 请将你的答案填写在下方
# 1、迭代次数是100时,明显模型和数据不拟合,把迭代次数加到10000时,数据模型和数据在前半个月基本是完全拟合;
# 2、在22号向后,数据模型和数据不太好,我猜的原因是不是节日期间的问题?如果多采集点节日期间的数据,修改模型,是不是这一块的拟合会变得更好.
# 
