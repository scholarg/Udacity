...
```
1、第一个测试测试了allFeeds变量不为空,包括URL和name;
2、测试菜单隐藏功能.菜单通过改变css来隐藏,只要模拟鼠标点击‘汉堡包’图标,并测试css是否改变就可以;
3、测试loadFeed函数调用,查询.feed元素下边的.entry-link里边包含.entry就可以.由于是异步加载,需要调用beforeEach和定义done().
4、测试加载新源内容会改变,只要获取到html,用toEqual来测试前后的html是否一致就可以.同样注意异步加载的测试.
5、在html文件调用测试程序的js文件就可以开启测试功能.
```
